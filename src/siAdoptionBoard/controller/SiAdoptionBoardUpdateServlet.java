 ...
 */
@WebServlet(name = "SiAdoptionBoardUpdate", urlPatterns = { "/siAdoptionBoardUpdate" })
public class SiAdoptionBoardUpdateServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SiAdoptionBoardUpdateServlet() {
        super();
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//기존 파일 경로
		String adoptionBoardOldFilename = mRequest.getParameter("adoptionBoardOldFilename");
		String adoptionBoardOldFilepath = mRequest.getParameter("adoptionBoardOldFilepath");
		//파일 삭제 로직
		String status = mRequest.getParameter("status");
		//파일 객체 생성
		File f = mRequest.getFile("adoptionBoardFilename");
		//파일 업로드 판단, 없으면 null
		if(f!=null && f.length()>0) {//첨부파일이 있는 경우
			if(adoptionBoardOldFilename != null) {
				//기존 파일을 삭제하고 새로운 첨부파일 추가
				File deleteFile = new File(saveDirectory+"/"+adoptionBoardOldFilepath);
				boolean bool = deleteFile.delete();
				System.out.println(bool?"삭제성공":"삭제실패");
				adoptionBoardOldFilename = adoptionBoardFilename;
				adoptionBoardOldFilename = adoptionBoardFilepath;
			}
		}else {//첨부파일이 없는 경우
			if(status.equals("stay")) {//새로운 첨부파일이 없고 기존의 파일을 삭제하지 않은 경우
				adoptionBoardFilename = adoptionBoardOldFilename;
				adoptionBoardFilepath = adoptionBoardOldFilename;
			}else {//삭제한 경우
				//파일 삭제 로직
				File deleteFile = new File(saveDirectory+"/"+adoptionBoardOldFilepath);
				boolean bool = deleteFile.delete();
		...
		int result = new AdoptionBoardService().adoptionBoardUpdate(adoptionBoardNo, adoptionBoardTitle, adoptionBoardContent, adoptionBoardFilename, adoptionBoardFilepath);
		String view = "";
		if(result>0) {
